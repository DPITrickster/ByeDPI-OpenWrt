...
- **Автор [изначального порта](https://github.com/spvkgn/ByeDPI-OpenWrt) ByeDPI для OpenWrt:** spvkgn — за `Makefile`, `init`-скрипты и оригинальные GitHub Actions, на которых всё базируется

### Заимствования и источники идей

Сборочная инфраструктура этого репозитория развивалась с оглядкой на родственные форки
ByeDPI-OpenWrt. Спасибо их авторам:

- [**1andrevich/ByeDPI-OpenWrt**](https://github.com/1andrevich/ByeDPI-OpenWrt) — подход к
  «резервным» (generic) пакетам адаптирован отсюда: скрипт `.github/build-byedpi.sh`
  (упаковка готовых бинарников через официальные `ipkg-build` + `apk mkpkg`, `postinst`/`conffiles`,
  сборка `apk-tools` из исходников, группировка по upstream-архитектуре).
- [**ronmount/ByeDPI-OpenWrt**](https://github.com/ronmount/ByeDPI-OpenWrt) — идеи по генерации
  индекса фида и кэшированию SDK.
- [**golovinengineer/ByeDPI-OpenWrt**](https://github.com/golovinengineer/ByeDPI-OpenWrt) — подход
  к генерации индекса штатной целью `make package/index` и матрице сборки.

> Базовая сборка пакета (`Makefile`, init-скрипт, патчи) восходит к порту **spvkgn**.
> Генерация индекса фида выполняется штатной целью OpenWrt SDK `make package/index`.
