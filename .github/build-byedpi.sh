#!/bin/bash
# Assemble IPK and APK packages for a given set of OpenWrt architectures from a
# pre-built upstream ciadpi binary. Uses the official OpenWrt ipkg-build and
# apk mkpkg tooling, and adds postinst (auto-enable service), prerm (disable)
# and conffiles handling — things the plain nFPM repack lacked.
#
# Usage: build-byedpi.sh <version> <binary_path> <"ow_arch1 ow_arch2 ...">
#   version      - package version, e.g. 0.17.3
#   binary_path  - path to the pre-built ciadpi binary
